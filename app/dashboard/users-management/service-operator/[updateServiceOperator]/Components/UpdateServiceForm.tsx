...
interface UserFormData {
  ...
  status: string;
  role: "ADMIN" | "SERVICEOPERATOR" | "CLIENT";
  Address: {
    addressLine1?: string;
    city?: string;
    country?: string;
...
const formSchema = z
  .object({
    ...
    role: z.enum(["ADMIN", "SERVICEOPERATOR", "CLIENT"], {
...
export default function UpdateAdminForm() {
  ...
  const userMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Service Operator updated successfully!");
      form.reset();
      setProfilePhoto(null);
      router.push("/dashboard/users-management/service-operator");
    },
    onError: (error) => {
  ...
  useEffect(() => {
    if (usersData?.data) {
      form.reset({
        ...
        role: usersData.data.role as "ADMIN" | "SERVICEOPERATOR" | "CLIENT" || 'ADMIN',
  ...
  return (
    <div className="m-4 space-y-4 rounded-sm bg-white p-4 pt-2">
      <Button
        variant="ghost"
        onClick={() => router.push("/dashboard/users-management/service-operator")}
      >
        <ArrowLeft className="mr-2 h-4 w-4" /> Back
      </Button>
      <h1 className="text-center text-2xl font-bold">Update Service Operator</h1>
