...
function ServiceOperatorPage() {
  ...
  const handleCreateNewCallOperator=()=>{
    router.push("/dashboard/users-management/service-operator/new-service-operator");
  }
...
// Get ambulance order data from API
  const getOrderData = async (page: number, limit: number) => {
    const response = await get<UsersManagementApiResponse>(`/users/user?page=${page}&limit=${limit}&role=SERVICEOPERATOR`, {
...
<div className='border-b-2 border-gray-200 pb-4 flex justify-between p-2'>
  <h1 className='font-bold text-[20px]'>Service Operator ({adminData?.rowCount})</h1>
  <Button className='bg-primary-500 text-white' onClick={handleCreateNewCallOperator}><LuCirclePlus />
  New Service Operator</Button>
</div>
      <TableData
        data={adminData?.data || []}
        isLoading={isLoading}
