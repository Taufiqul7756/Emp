...
export default function UpdateAdminForm() {
  ...
  const id = params?.updateClient || params?.updateClient;
  ...
  const userMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push("/dashboard/users-management/client");
    },
    onError: (error) => {
  ...
  return (
    <div className="m-4 space-y-4 rounded-sm bg-white p-4 pt-2">
      <Button
        variant="ghost"
        onClick={() => router.push("/dashboard/users-management/client")}
      >
        <ArrowLeft className="mr-2 h-4 w-4" /> Back
      </Button>
      <h1 className="text-center text-2xl font-bold">Update Client</h1>
