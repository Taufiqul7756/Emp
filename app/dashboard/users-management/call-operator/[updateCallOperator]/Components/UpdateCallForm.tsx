...
interface UserFormData {
  ...
  status: string;
  role: "ADMIN" | "CALLOPERATOR" | "CLIENT";
  Address: {
    addressLine1?: string;
    city?: string;
    country?: string;
...
const formSchema = z
  .object({
    ...
    role: z.enum(["ADMIN", "CALLOPERATOR", "CLIENT"], {
...
export default function UpdateCallForm() {
  const { data: session } = useSession();
  const params = useParams();
  // console.log("params", params);
  const id = params?.updateCallOperator || params?.updateCallOperator;
  ...
  // console.log("get signle user data---", usersData);
  ...
  const userMutation = useMutation({
    mutationFn: async (data: UserFormData) => {
      const updatedData: UserFormData = { 
        ...data, 
        Profile: {
          profilePhoto: profilePhoto ? profilePhoto : data?.Profile?.profilePhoto,
    ...
    onSuccess: () => {
      toast.success("Call Operator updated successfully!");
      form.reset();
      setProfilePhoto(null);
      router.push("/dashboard/users-management/call-operator");
    },
    onError: (error) => {
  ...
  useEffect(() => {
    if (usersData?.data) {
      form.reset({
        ...
        role: usersData.data.role as "ADMIN" | "CALLOPERATOR" | "CLIENT" || 'ADMIN',
  ...
  return (
    <div className="m-4 space-y-4 rounded-sm bg-white p-4 pt-2">
      <Button
        variant="ghost"
        onClick={() => router.push("/dashboard/users-management/call-operator")}
      >
        <ArrowLeft className="mr-2 h-4 w-4" /> Back
      </Button>
      <h1 className="text-center text-2xl font-bold">Update Call Operator</h1>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card className="w-full">
            <CardHeader className="flex flex-row items-center justify-between rounded-tl-lg rounded-tr-lg border-b bg-gray-50">
              ...
              <Checkbox
                ...
                className="h-5 w-5 border-red-500 bg-primary-100 data-[state=checked]:border-red-500 data-[state=checked]:bg-red-500 data-[state=checked]:text-white"
              />
            </CardHeader>
