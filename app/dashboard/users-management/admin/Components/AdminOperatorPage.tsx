...
import { AmbulanceAdminApiResponse } from '../types/types';
import { TableData } from './TableData';
...
function AdminOperatorPage() {
  ...
  const handleCreateNewCallOperator=()=>{
    router.push("/dashboard/users-management/admin/new-admin");
  }
...
// Get ambulance order data from API
  const getOrderData = async (page: number, limit: number) => {
    const response = await get<UsersManagementApiResponse>(`/users/user?page=${page}&limit=${limit}&role=ADMIN`, {
...
<div className='border-b-2 border-gray-200 pb-4 flex justify-between p-2'>
  <h1 className='font-bold text-[20px]'>Admin ({adminData?.rowCount})</h1>
